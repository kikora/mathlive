...
printf "${LINECLEAR}${DOT} Building TypeScript declaration files (.d.ts)"
npx tsc --project ./tsconfig.json --declaration --emitDeclarationOnly --outDir ./declarations
mv ./declarations/src/public ./dist
rm -rf ./declarations
echo -e "$LINECLEAR$CHECK TypeScript declaration files built${RESET}"
...
node --experimental-json-modules ./scripts/watch.mjs
